= DevDesk Email Integration Setup
:toc: left
:toclevels: 3
:icons: font
...
DevDesk integrates with Exchange Online to provide:

* **Inbound**: Customers email devdesk@knowall.ai to create tickets
...
4. Set display name: "DevDesk Support"
5. Set email: devdesk@knowall.ai
6. Click "Create"
...
Set-CASMailbox -Identity "devdesk@knowall.ai" -SmtpClientAuthenticationDisabled $false

# Verify
Get-CASMailbox -Identity "devdesk@knowall.ai" | Select SmtpClientAuthenticationDisabled
----
...
1. **Trigger**: "When a new email arrives (V3)"
   - Folder: Inbox
   - To: devdesk@knowall.ai

2. **Action**: "HTTP"
   - Method: POST
   - URI: https://devdesk.knowall.ai/api/email/webhook
   - Headers:
...
SMTP_PORT=587
SMTP_USER=devdesk@knowall.ai
SMTP_PASSWORD=<app-password>
SMTP_FROM=devdesk@knowall.ai
SMTP_FROM_NAME=DevDesk Support
...
* Link back to ticket in DevDesk
* Requester's original message (quoted)
* Agent's response

Example format:
----
Subject: Re: [DevDesk #123] Original Subject
...
Ticket: #123
Status: {Status}
View online: https://devdesk.knowall.ai/tickets/123
...
[source,bash]
----
curl -X POST https://devdesk.knowall.ai/api/email/webhook \
...
# Via API
curl -X POST https://devdesk.knowall.ai/api/devops/tickets/123/reply \
