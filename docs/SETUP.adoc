...
DevDesk is a Zendesk-style support ticketing portal that uses Azure DevOps as its backend. Work items tagged with `ticket` are displayed as support tickets.
...
** Redirect URI: `https://devdesk.knowall.ai/api/auth/callback/azure-ad` (Web)
...
. Add CNAME record: `devdesk` → `devdesk-knowall.azurewebsites.net`
. Add custom domain to App Service:
+
[source,bash]
----
az webapp config hostname add \
  --webapp-name devdesk-knowall \
  --resource-group KnowAllAIRG \
  --hostname devdesk.knowall.ai
----
...
|Azure Web App publish profile (XML). Get with: `az webapp deployment list-publishing-profiles --name devdesk-knowall --resource-group KnowAllAIRG --xml`
...
Configure your email provider to forward emails to `devdesk@knowall.ai` via webhook:

* Webhook URL: `https://devdesk.knowall.ai/api/email/webhook`
